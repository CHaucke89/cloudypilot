...
class ModelWidget(Widget):
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    if model_manager and model_manager.activeBundle.ref:
      active_name = model_manager.activeBundle.internalName
    else:
...
class SetupWidget(Widget):
  def __init__(self):
    ...
    self._model_widget = ModelWidget() if gui_app.sunnypilot_ui() else None
  ...
  def set_open_models_callback(self, callback):
    if self._model_widget:
      self._model_widget.set_open_models_callback(callback)

  def _render(self, rect: rl.Rectangle):
    if not ui_state.prime_state.is_paired():
      self._render_registration(rect)
    elif self._model_widget:
      self._model_widget.render(rect)
    else:
